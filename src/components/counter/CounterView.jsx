...
const CounterView = ({ habit, onClose }) => {
    ...
    return (
        <div className="flex items-center justify-center p-4 h-full w-full bg-bg/60 backdrop-blur-md animate-fade-in fixed inset-0 z-50">

            {/* Main Card */}
            <div className="glass-panel rounded-3xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col relative animate-scale-in border border-white/50">

                {/* Header */}
                <div className="p-5 border-b border-border flex items-center justify-between bg-surface/50 backdrop-blur-md sticky top-0 z-10">
                    <div className="flex items-center gap-3">
                        ...
                        />
                        <div>
                            <h2 className="font-bold text-text text-lg leading-tight">{habit.name}</h2>
                    ...
                    <button
                        onClick={onClose}
                        className="w-9 h-9 rounded-full bg-surface-light hover:bg-surface text-text-secondary hover:text-text flex items-center justify-center transition-all duration-200 active:scale-95 border border-border"
                    >
                        <X size={20} />
                    </button>
                </div>
                ...
                {/* Body */}
                <div className="p-8 flex flex-col items-center justify-center gap-8 relative bg-surface/30">
                    ...
                    <div className="relative group cursor-pointer" onClick={habit.type === 'time' ? toggleTimer : handleIncrement}>
                        ...
                        <svg className="transform -rotate-90 w-72 h-72 drop-shadow-xl">
                            ...
                            <circle
                                ...
                                strokeWidth="16"
                                fill="transparent"
                                className="text-surface-light opacity-50"
                            />
                            {/* Progress */}
                            <circle
                                cx="144"
                                cy="144"
                                r={radius}
                                stroke={habit.color} // Dynamic habit color
                                strokeWidth="16"
                                fill="transparent"
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center select-none">
                            <span className="text-6xl font-bold text-text tabular-nums tracking-tight transition-transform duration-100 active:scale-95">
                                {habit.type === 'time' ? formatTime(timer) : count}
                            </span>
                            <span className="text-sm text-text-secondary font-semibold mt-2 uppercase tracking-wide">
                        ...
                        {habit.type === 'count' && (
                            <>
                                <button
                                    ...
                                    className="w-14 h-14 rounded-2xl bg-surface-light border border-border hover:bg-surface text-text-secondary hover:text-text flex items-center justify-center transition-all active:scale-95"
                                >
                                    <ChevronDown size={28} />
                                </button>
                                ...
                                <button
                                    ...
                                    className={`w-20 h-20 rounded-2xl flex items-center justify-center transition-all shadow-lg hover:shadow-xl hover:-translate-y-1 active:translate-y-0 active:scale-95 ${isAutoCounting
                                        ...
                                        : 'bg-surface border-2 border-primary/20 text-primary hover:border-primary'
                                        }`}
                                >
                                    {isAutoCounting ? (
                                ...
                                <button
                                    ...
                                    className="w-14 h-14 rounded-2xl bg-primary text-white shadow-lg shadow-primary/30 hover:bg-primary-dark flex items-center justify-center transition-all hover:-translate-y-1 active:translate-y-0 active:scale-95"
                                >
                                    <ChevronUp size={28} />
                                </button>
                            </>
                ...
                {/* Footer Stats */}
                <div className="bg-surface-light/50 border-t border-border grid grid-cols-3 divide-x divide-border">
                    <div className="p-4 text-center group cursor-default">
                        <span className="block text-[10px] text-text-secondary uppercase font-bold tracking-wider mb-1">Current Streak</span>
                    ...
                    <div className="p-4 text-center group cursor-default">
                        <span className="block text-[10px] text-text-secondary uppercase font-bold tracking-wider mb-1">Goal Progress</span>
                        <div className={`font-bold text-lg group-hover:scale-110 transition-transform text-primary`}>
                    ...
                    >
                        <span className="block text-[10px] text-text-secondary group-hover:text-red-400 uppercase font-bold tracking-wider mb-1 transition-colors">Reset</span>
                        <div className="flex items-center justify-center text-text-secondary group-hover:text-red-500 transition-colors">
